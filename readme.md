...
0. 环境要求：php 7.0以上，推荐7.0至8.0（超过8.0版本尚未充分测试）。必须开启curl和socket两项扩展，并将php.ini中output_buffering一项打开（一般设为output_buffering = 4096）。必须支持本地对本地php文件的http访问，并准备一个不需经过dns解析的本地文件地址（类似于http://127.0.0.1/dts）。建议将php所需内存修改为256M或更高，建议将php最大执行时间修改为180秒或更高。
