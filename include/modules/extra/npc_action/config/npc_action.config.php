...
{
	$npc_action_gametype = Array(0, 4, 6, 10, 13, 14, 15, 16, 18, 19, 20);//开启NPC行动的游戏模式
	...
	$npc_action_data = Array(//定义哪些NPC会自动行动。
		...
		'芙蓉' => Array(
			...
			'setting' => Array(//行动设定。
				...
				'chase' => Array(
					...
					'addchat' => 1,//是否发送聊天记录
					'addchat_txt' => Array(//发送特定的聊天记录，用<:para1:>和<:para2:>代表出发地和目的地，用<:para3:>代表追踪目标
						'这里是芙蓉，正在追击危险玩家：<:para3:>，准备前往<:para2:>，完毕。',
					),
				),
				'ambush' => Array(//问候前允许偷袭一次！就算不是探索也有概率先制玩家，但需要在同一个地图
					'early_action' => 1,//是否为提前批行动，如果是则不满足条件后还会再执行一遍其他行动
					...
					'addchat_txt' => Array(//发送特定的聊天记录，用<:para1:>和<:para2:>代表出发地和目的地，用<:para3:>代表追踪目标
						'问候之前允许偷袭一次！',
						'这里是芙蓉，正在偷袭危险玩家：<:para3:>。什么？当然是完成了才会报告的，完毕。',
					),
				),
			),
		),	
		...
		'测试NPC' => Array(
			'intv' => 120,//行动间隔
			'devi' => Array(-60, 30),//行动间隔偏差值，第一个数值为负偏差，第二个数值为正偏差，会在范围内随机取值
			'actions' => Array(//会执行的行动及对应的概率。具体执行时先取满足条件的行动，然后根据比例来判定概率。可选项有move（随机移动）、chase（追杀）、evade（躲避）
				'chase' => 100,
				'ambush' => 100,
			),
			...
			'setting' => Array(//行动设定。
				...
				'chase' => Array(
					'object' => Array('R'),//追逐对象，R为随机选一个玩家，T为追头名，B为追最弱的，P:XXX为追踪名字为XXX的玩家，N:XXX为追踪名字为XXX的NPC，W为追上一次与自己作战的玩家
					'avoid_forbidden' => 0,//是否躲避禁区
					'avoid_dangerous' => 0,//是否躲避危险地图
					//'need_awake' => 1,//是否在清醒状态（先制任意玩家后）才会开始执行本项行动
					// 'need_rage_GE' => 30,//怒气最少要到达哪个值才会开始执行本行动
				...
				'ambush' => Array(//问候前允许偷袭一次！就算不是探索也有概率先制玩家，但需要在同一个地图
					'early_action' => 1,//是否为提前批行动，如果是则不满足条件后还会再执行一遍其他行动
					'object' => Array('W'),//偷袭对象，A为不分条件，S为偷袭较强的，L为偷袭较弱的，P:XXX为名字为XXX的玩家，W为偷袭上一次与自己作战的玩家
					//'need_awake' => 1,//是否在清醒状态（先制任意玩家后）才会开始执行本项行动
					'need_rage_GE' => 31,//怒气最少要到达哪个值才会开始执行本行动
					'rage_change_after_action' => -1,//执行后怒气变化量，可正可负
