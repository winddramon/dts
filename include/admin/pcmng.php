...
if($command == 'kill' || $command == 'live' || $command == 'del') {
	...
	if($operlist || $operlist2 || $dfaillist || $gfaillist){
		if($command == 'kill'){
			...
			$operword = '被复活';
			$qryword = "UPDATE {$tablepre}players SET player_dead_flag='0', hp=mhp,state='0',endtime='$now' ";
		}elseif($command == 'del'){
			$operword = '被清除';
